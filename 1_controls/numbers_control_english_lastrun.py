...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\micha\\numerals\\1_controls\\numbers_control_english_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    Welcome_Text = visual.TextStim(win=win, name='Welcome_Text',
        ...
        languageStyle='LTR',
        depth=0.0);
    Welcome_Continue_mouse = event.Mouse(win=win)
    x, y = [None, None]
    ...
    Welcome_Screen = data.Routine(
        name='Welcome_Screen',
        components=[Welcome_Text, Welcome_Continue_mouse],
    )
    Welcome_Screen.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # setup some python lists for storing info about the Welcome_Continue_mouse
    Welcome_Continue_mouse.x = []
    Welcome_Continue_mouse.y = []
    Welcome_Continue_mouse.leftButton = []
    Welcome_Continue_mouse.midButton = []
    Welcome_Continue_mouse.rightButton = []
    Welcome_Continue_mouse.time = []
    ...
    while continueRoutine:
        ...
        if Welcome_Text.status == STARTED:
            # update params
            pass
        # *Welcome_Continue_mouse* updates
        
        # if Welcome_Continue_mouse is starting this frame...
        if Welcome_Continue_mouse.status == NOT_STARTED and t >= 3-frameTolerance:
            # keep track of start time/frame for later
            Welcome_Continue_mouse.frameNStart = frameN  # exact frame index
            Welcome_Continue_mouse.tStart = t  # local t and not account for scr refresh
            Welcome_Continue_mouse.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(Welcome_Continue_mouse, 'tStartRefresh')  # time at next scr refresh
            # update status
            Welcome_Continue_mouse.status = STARTED
            Welcome_Continue_mouse.mouseClock.reset()
            # if button is down already this ISN'T a new click
            Welcome_Continue_mouse.prevButtonState = Welcome_Continue_mouse.getPressed()
        if Welcome_Continue_mouse.status == STARTED:  # only update if started and not finished!
            buttons = Welcome_Continue_mouse.getPressed()
            # button state changed?
            if buttons != Welcome_Continue_mouse.prevButtonState:
                Welcome_Continue_mouse.prevButtonState = buttons
                # state changed to a new click
                if sum(buttons) > 0:
                    pass
                    x, y = Welcome_Continue_mouse.getPos()
                    Welcome_Continue_mouse.x.append(float(x))
                    Welcome_Continue_mouse.y.append(float(y))
                    buttons = Welcome_Continue_mouse.getPressed()
                    Welcome_Continue_mouse.leftButton.append(buttons[0])
                    Welcome_Continue_mouse.midButton.append(buttons[1])
                    Welcome_Continue_mouse.rightButton.append(buttons[2])
                    Welcome_Continue_mouse.time.append(Welcome_Continue_mouse.mouseClock.getTime())
    ...
    thisExp.addData('Welcome_Continue_mouse.leftButton', Welcome_Continue_mouse.leftButton)
    thisExp.addData('Welcome_Continue_mouse.midButton', Welcome_Continue_mouse.midButton)
    thisExp.addData('Welcome_Continue_mouse.rightButton', Welcome_Continue_mouse.rightButton)
    thisExp.addData('Welcome_Continue_mouse.time', Welcome_Continue_mouse.time)
    # Run 'End Routine' code from reset_global_clock
    globalClock.reset()
    thisExp.nextEntry()
